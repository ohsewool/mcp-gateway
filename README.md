...
python3 -m pytest tests/ -q                      # 185 tests
...
결정은 메모리에만 남으면 소용이 없다. 며칠 뒤 "무엇을 시도했고 무엇이 막혔고 통과한 하나는 누가 승인했나"를 묻는 사람은 그 자리에 없었다. 해시 체인 JSONL로 append-only 기록하며, 코어의 증적 export와 같은 형식이라 하나의 검증기로 읽힌다. 재시작하면 기존 체인을 이어 쓴다.
